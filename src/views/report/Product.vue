	...
	<main-section>
		...
		<card-component title="Produk" :icon="mdiMonitorCellphone" has-table>
			<table>
				...
				<tbody>
					<tr v-for="(product, index) in allProducts.result" :key="index">
						<td>{{ product.kode }}</td>
						<td>{{ product.nama }}</td>
						<td>{{ product['operator.nama'] }}</td>
						<td>{{ currency(product['markup_produk.markup']) }}</td>
						<td class="is-actions-cell">
							<jb-buttons type="justify-start lg:justify-end" no-wrap>
								<jb-button
									...
									:icon="mdiPencil"
									small
									@click="toggleModal(product.kode)"
								/>
							</jb-buttons>
						</td>
					</tr>
...
export default {
	...
	methods: {
		...
		toggleModal: function (kode) {
			...
			this.$store.dispatch('createupdateProduct', data);
			this.retrieveProducts();
		},
